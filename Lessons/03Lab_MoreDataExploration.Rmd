...
```{r DataExercise-1a}
durham_10day_highs <- c(82,83,84,79,71,78,82,85,84,82)
durham_10day_lows <- c(56,58,60,60,62,62,64,64,62,60)
```
...
```{r DataExercise-1b}
durham_10day_highs_celsius <- ((durham_10day_highs - 32) * 5/9)
durham_10day_lows_celsius <- ((durham_10day_lows - 32) * 5/9)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
durham_10day_dataframe <- data.frame(durham_10day_highs, durham_10day_highs_celsius, durham_10day_lows, durham_10day_lows_celsius)
names(durham_10day_dataframe) <- c("Highs in F", "Highs in C", "Lows in F", "Lows in C")
```
...
```{r DataExercise-1d}
summary(durham_10day_dataframe)
```
